...
python launch_ray_adder_async.py --workers=2 --cluster=local

# launch on AWS using given instance type
python launch_ray_adder_async.py --workers=2 --cluster=aws --instance=c5.18xlarge
"""
...
def launch(backend, install_script='', init_cmd=''):
  ...
  head_task.upload('ray_adder_async.py')
  ...
  client_cmd = 'python ray_adder_async.py --redis-address %s:%d --size-mb %d'%(head_task.ip, DEFAULT_PORT, args.size_mb)
  ...
  job.tasks[0].stream_file('/tmp/log.txt')
