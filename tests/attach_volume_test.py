...
def main():
  ...
  job = run.make_job('volattach2', instance_type=args.instance_type,
                     ami_name=args.ami_name,
                     availability_zone=args.zone,
                     linux_type=args.linux_type)
  job.wait_until_ready()
  vol = u.lookup_volume('imagenet_0')
  u.attach_volume(vol, job.instance)
  u.mount_volume(vol, job.tasks[0], '/data')
  result = job.run_and_capture_output('ls /data')
