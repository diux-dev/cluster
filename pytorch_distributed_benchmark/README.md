...
To setup your environment (conda env, PATH, AWS keys), follow instructions for gpubox: https://github.com/diux-dev/cluster/tree/master/gpubox
...
# create local local env to match one used on DLAMI
conda env remove --name pytorch_p36 -y
conda create --name pytorch_p36 --clone nexus

python bench_p2p.py --run-locally
```
This creates tmux session with default name p2p (--name argument) and starts the exchange. To attach and see the progress
...
This allocates two default instance types (--instance-type argument), with names `0.worker.p2p`, `1.worker.p2p` and starts the benchmark on them. You can connect to one of the instances using `ssh` instructions printed. Look for something like this in console
...
```
python bench_p2p.py --name=c5 --placement=1 --instance-type=c5.18xlarge --zone=us-west-2c
connect c5
